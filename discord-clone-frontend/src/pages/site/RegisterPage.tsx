...
const RegisterPage: React.FC = () => {
  ...
  const [birthMonth, setBirthMonth] = useState(""); // New state for birth month
  const [birthDay, setBirthDay] = useState(""); // New state for birth day
  const [birthYear, setBirthYear] = useState(""); // New state for birth year

  const currentYear = new Date().getFullYear(); // Get the current year
  const minYear = currentYear - 100; // Calculate the minimum year (100 years back)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const dataRequest: RegisterData = {
        ...
        //birthday: `${birthMonth}-${birthDay}-${birthYear}`, // Format birthday
      };
      ...
      <div className="bg-gray-800 text-white p-8 rounded-lg shadow-md max-w-md w-full">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-4">
            <label
              htmlFor="email"
              className="block text-sm font-bold mb-1 text-gray-300"
            >
              Email
            </label>
            <input
          ...
          <div className="mb-4 flex space-x-2">
            <label
              htmlFor="birthMonth"
              className="block text-sm font-bold mb-1 text-gray-300 text-center pt-2" // Added pt-2 for top padding
            >
              Date of Birth
            </label>
            ...
            <div className="flex-1">
              <input
                ...
                className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="Year"
                min={minYear} // Minimum value for year
                max={currentYear} // Maximum value for year
                required
              />
            </div>
          </div>
